...
const SchemaPanel = ({ hide }) => {
    ...
    return <Panel style={{display: hide ? "none" : ""}}bordered header="Schema Editor">
        <Button appearance="primary" onClick={() => { loadSchema(schema, () => { dispatch({ type: "new" }) }, setAlert) }}>Create new schema</Button>
        <SchemaLoader />
        {schema && <>
            <Button onClick={() => {
                setAlert("Do you want to clear the current schema? Unsaved progress will be erased.", "modal", "Clearing schema", {
                    yes: "Continue",
            ...
            <Input placeholder="Schema name" onChange={(value) => dispatch({ type: "name", payload: value })} value={schema.name} />
            <InputNumber
            ...
            />
            <Button appearance="primary">Save schema to server</Button>
            <Button appearance="primary" onClick={() => {downloadSchema(schema, setAlert)}}>Download schema</Button>
        </>}
    </Panel>
}
